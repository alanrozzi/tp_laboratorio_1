	...
	while(reintentos>=0)
	{
		...
		if(getNombre(bufferString,sizeof(bufferString)) == 0 && strnlen(bufferString,sizeof(bufferString)) < longitud )
		{
			for(int i=0;i<longitud;i++)
			{
				if(bufferString[i]==' ')
				{
					bufferString[i+1]=toupper(bufferString[i+1]);
				}
			}
 ...
 *
 */
int utn_getCaracterMAYUS(char* pResultado, char* mensaje, char* mensajeError, char minimo, char maximo, int reintentos)
{
	int retorno=-1;
	char bufferChar;
